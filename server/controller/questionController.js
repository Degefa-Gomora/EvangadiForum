...
async function postQuestion(req, res) {
  const { user_id, title, description, tag } = req.body;
  ...
  try {
    await dbConnection.query(
      "insert into questions (questionid, user_id, title, description, tag,createdAt) values ( ?, ?, ?, ?, ?,?)",
      [questionid, user_id, title, description, tag, formattedTimestamp]
    );
    return res
...
// get single question and answers (updated to fetch q.user_id and solution_answer_id)
async function getQuestionAndAnswer(req, res) {
  ...
  try {
    const [questionRows] = await dbConnection.query(
      `SELECT
          q.questionid,
          q.user_id AS qtn_user_id, -- Fetch the question owner's ID
          q.title,
          q.description,
...
async function markAnswerAsSolution(req, res) {
  ...
  const loggedInuser_id = req.user.user_id; // User ID from authMiddleware
  ...
  try {
    ...
    const [questionRows] = await dbConnection.query(
      `SELECT user_id FROM questions WHERE questionid = ?`,
      [questionId]
    );
    ...
    if (questionOwnerId !== loggedInuser_id) {
